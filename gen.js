...
const VITAL={
sensory:['风把一个塑料袋吹到脚边，转了半圈才停。','头顶的灯闪了两下，像在打暗号。','空气里有油烟味，混着一点雨。','邻座的汤太烫，说话都带着哈气。','地上一摊没干的水渍，映着半个招牌。','空调滴水，滴在铁皮上，一声，一声。','阳光斜进来，把灰尘照得清清楚楚。','墙上的福字褪了色，倒着贴。','桌腿底下垫着半块砖，桌子不晃了。','玻璃上全是手印，最小的那枚在最上面。','烟灰缸满了，最上面那根还冒着烟。','一道光从没拉严的窗帘缝里切进来，正好切在脸上。'],
micro:['她把杯子转了半圈，又转回去。','他把烟夹在指间，忘了点。','她笑到一半，笑不动了。','他用鞋尖碾了碾地上的烟头。','她把头发别到耳后，想了想，又放下来。','他数了三遍零钱，数出三个不同的数。','她把围巾往上拉了拉，又扯回原位。','他靠着墙，慢慢出溜下去半寸。','她把手机扣在桌上，屏幕朝下。','他打了个哈欠，打到一半变成叹气。','她用指甲刮了下杯沿，声音很小。','他把袖子卷上去，看了一眼，又放下来。'],
...
sound:['冰柜的嗡嗡声忽然大了一拍。','远处有人放了一挂鞭炮，就三响。','楼上的脚步声，停在第七级台阶。','外卖柜「哐当」一声合上。','谁的手机在包里震，没人接。','隔壁电视在放天气预报。','卷帘门拉到一半，卡住了。','热水壶按下去，半天没响。','电梯到了，门开了，没人。','雨点砸遮阳棚，先是一下，然后连成片。','耳机里漏出半句歌，走调的。','风铃响了一声。没有风。'],
...
window.freshArticle=function(target){
  ...
  // 对话片段池：让文章里偶尔有直接引语，不是"他说""我说"而是原话
  const DIALOGUE = [
    c=>'「'+pick(['行了。','嗯。','别说了。','也好。','再说吧。','就这样吧。','也好。'])+'」',
    c=>'「'+pick(['那不一样。','这你就不懂了。','你想多了。','算了算了。','别说这个了。'])+'」',
    c=>'「'+pick(['你这个人……','怎么跟你说不明白呢。','行，我不说了。','随你吧。'])+'」',
    c=>'「'+pick(['我不知道。','我没想那么多。','就这样。','就这样吧。','哎……'])+'」',
    c=>'「'+pick(['你图什么？','谁让你管的？','这事儿跟你没关系。','管好你自己。'])+'」',
  ...
  // 短段落池：真实写作里大量 50-100 字的碎片观察，不全是 150+ 字"完整段"
  const SHORT_V = [
    c=>'那天特别冷。',
    c=>'他说他没想过这些。',
    c=>'其实我也不确定。',
  ...
  do {
    ...
    // 段落文本：模拟真实写作的不规则感，但保留叙事完整性
    let body = '';
    const shortUsed = new Set(); // 避免连续用同一个短段落
    ...
    ordered.forEach((s, idx) => {
      ...
      // 【非首尾段】18% 概率做短段落（保持完整一句，不拼接）
      if(!isLead && !isClose && Math.random() < 0.18){
        let tries = 0, shortPara;
        do {
          shortPara = pick(SHORT_V)(c);
          tries++;
        } while(shortUsed.has(shortPara) && tries < 5);
        shortUsed.add(shortPara);
        body += shortPara + '\n\n';
        return;
      }

      // 【非首尾段】28% 概率口语开头（自然接续，不是打断）
      if(!isLead && !isClose && Math.random() < 0.28){
        para = pick(ASIDE) + para.charAt(0).toLowerCase() + para.substring(1);
      }

      // 【中间段】28% 概率在段中或段尾插生活细节
      if(!isLead && !isClose && Math.random() < 0.28){
      ...
      // 【长段落切短】超过 200 字在句号处断成两段
      if(para.length > 200 && Math.random() < 0.45){
